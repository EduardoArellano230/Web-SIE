...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        $rutaRelativa = "../imagenes_ofertas/" . $nombreFinal;

        if (!is_dir("../imagenes_ofertas")) {
            mkdir("../imagenes_ofertas", 0777, true);
        }
        ...
        if (move_uploaded_file($imagenTmp, $rutaRelativa)) {
            try {
                ...
                $stmt = $bd->prepare("INSERT INTO ofertas (imagen, titulo, descripcion, categoria) VALUES (?, ?, ?, ?)");
                $stmt->execute([
                    $nombreFinal,
                    $titulo,
                    $descripcion,
